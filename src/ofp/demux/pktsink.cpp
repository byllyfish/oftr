...
bool PktSink::openFile(const std::string &file) {
    ...
    pcap_ = pcap_open_dead_with_tstamp_precision(DEFAULT_LINK_TYPE, DEFAULT_SNAP_LEN, PCAP_TSTAMP_PRECISION_NANO);
    if (!pcap_) {
...
void PktSink::write(const Timestamp &ts, const ByteRange &captureData, UInt32 length) {
