...
FlowData FlowState::receive(const Timestamp &ts, UInt32 end,
  ...
  if (Segment::lessThan(begin, end_)) {
    log::warning("FlowState:", end_, "drop early segment",
                 segmentStr(begin, end));
    return FlowData{};
  }
  ...
  if (seg && seg->begin() == end_) {
    log::debug("FlowState:", end_, "return flow data",
               segmentStr(seg->begin(), seg->end()));
    return FlowData{this, seg->data(), sessionID, true};
  }

  log::warning("Flowstate:", end_, "current cached begin", begin,
               "does not match flow end");
