...
OXMRange MatchHeader::oxmRange() const {
    ...
    return OXMRange{BytePtr(this) + sizeof(MatchHeader), length() - sizeof(MatchHeader)};
}
...
bool MatchHeader::validateInput(size_t lengthRemaining) const {
    ...
    OXMRange oxm{BytePtr(this)+sizeof(MatchHeader), matchLength - sizeof(MatchHeader)};
    ...
    if (matchLengthPadded > matchLength) {
        if (!IsMemFilled(BytePtr(this) + matchLength, matchLengthPadded - matchLength, 0)) {
