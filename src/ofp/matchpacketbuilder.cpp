...
const UInt64 kRequiredEthernet = FieldBit(OFB_ETH_DST) | FieldBit(OFB_ETH_SRC) | FieldBit(OFB_ETH_TYPE);

const UInt64 kRequiredArp = kRequiredEthernet | FieldBit(OFB_ARP_OP) | FieldBit(OFB_ARP_SHA) | FieldBit(OFB_ARP_SPA) |
    FieldBit(OFB_ARP_THA) | FieldBit(OFB_ARP_TPA);

const UInt64 kRequiredLLDP = kRequiredEthernet | FieldBit(X_LLDP_CHASSIS_ID) | FieldBit(X_LLDP_PORT_ID) | FieldBit(X_LLDP_TTL);
...
void MatchPacketBuilder::reportMissingFields() const {
    log_info("MatchPacketBuilder: Missing Fields!", log::hex(present_), log::hex(kRequiredArp));
}
