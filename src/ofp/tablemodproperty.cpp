...
bool TableModPropertyValidator::validateInput(const PropertyRange &range, Validation *context) {
  for (const auto &prop : range) {
    switch (prop.type()) {
      ...
      case TableModPropertyExperimenter::type():
        if (!prop.property<TableModPropertyExperimenter>().validateInput(context))
          return false;
        break;
    }
  }
