...
UInt64 Driver::connect(ChannelMode mode, UInt64 securityId, const IPv6Endpoint &remoteEndpoint, 
  ProtocolVersions versions, ChannelListener::Factory listenerFactory, 
  std::function<void(Channel*,std::error_code)> resultHandler) {
  return engine_->connect(mode, securityId, remoteEndpoint, versions, listenerFactory, resultHandler);
}

UInt64 Driver::connectUDP(ChannelMode mode, UInt64 securityId, const IPv6Endpoint &remoteEndpoint, 
  ProtocolVersions versions, ChannelListener::Factory listenerFactory, 
  std::error_code &error) {
    return engine_->connectUDP(mode, securityId, remoteEndpoint, versions, listenerFactory, error);
}
