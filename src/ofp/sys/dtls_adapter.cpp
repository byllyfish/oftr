...
DTLS_Adapter::DTLS_Adapter(asio::ssl::context *context, DeliverFunc sendCallback, DeliverFunc receiveCallback, void *userData)
: sendCallback_{sendCallback}, receiveCallback_{receiveCallback}, userData_{userData}
 {
...
bool DTLS_Adapter::isHandshakeDone() const {
    log::debug("isHandshakeDone: state", SSL_state_string_long(ssl_), ERR_peek_last_error());
    ...
    if (rc <= 0) {
        ...
        switch (err) {
            ...
            case SSL_ERROR_WANT_WRITE:
                ...
                log::warning("DTLS_Adapter::sendDatagram: unexpected SSL_ERROR_WANT_WRITE");
                break;
        ...
        log::debug("sendDatagram: SSL_get_error returned", err, "state", SSL_state_string_long(ssl_));
    }
...
void DTLS_Adapter::datagramReceived(const void *datagram, size_t length) {
    ...
    log::debug("datagramReceived: BIO_write returned", rc, ByteRange{datagram, length});
    ...
    if (rc > 0) {
        ...
        int err = SSL_get_error(ssl_, rc);
        log::debug("datagramReceived: SSL_get_error returned", err, "state", SSL_state_string_long(ssl_), ERR_peek_last_error());
    }
