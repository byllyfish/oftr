...
static const char *dtlsTypeString(UInt8 type) {
  switch (type) {
    ...
    default: return "DTLS_unknown_type";
  }
}
...
static const char *dtlsVersionString(UInt32 version) {
  switch (version) {
    ...
    case 0xFEFD: return "DTLS1.2";
    default: return "DTLS_unknown_version";
  }
}
...
static const char *dtlsHandshakeType(UInt8 type) {
    switch (type) {
        ...
        case 23: return "supplemental_data";
        case 67: return "next_proto";
        case 203: return "encrypted_extensions";
        default: return "DTLS_unknown_handshake_type";
    }
}
...
std::string ofp::sys::DTLS_PrintRecord(const void *record, size_t length) {
    ...
    UInt64 recordSeqNum = (UInt64_cast(data[5]) << 5*8) |
                            (UInt64_cast(data[6]) << 4*8) |
                            (UInt64_cast(data[7]) << 3*8) |
                            (UInt64_cast(data[8]) << 2*8) |
                            (UInt64_cast(data[9]) << 1*8) | 
                            UInt64_cast(data[10]);

    std::stringstream stream;

    stream << dtlsVersionString(recordVersion) << ' ' << dtlsTypeString(recordType) << '(' << static_cast<int>(recordType) << ')' << " len=" << recordLen << " epoch=" << recordEpoch << " seq=" << recordSeqNum;
    ...
    if (recordEpoch > 0) {
        ...
        if (recordLen >= 25) {
            ...
            UInt32 hsLength = (UInt32_cast(data[14]) << 16) |
                                    (UInt32_cast(data[15]) << 8) |
                                    UInt32_cast(data[16]);
            UInt32 hsMsgSeq = (UInt32_cast(data[17]) << 8) | data[18];
            UInt32 hsFragmentOffset = (UInt32_cast(data[19]) << 16) |
                                    (UInt32_cast(data[20]) << 8) | UInt32_cast(data[21]);
            UInt32 hsFragmentLength = (UInt32_cast(data[22]) << 16) |
                                    (UInt32_cast(data[23]) << 8) | UInt32_cast(data[24]);
            stream << ' ' << dtlsHandshakeType(hsType) << '(' << static_cast<int>(hsType) << ')';
            stream << " msgLen=" << hsLength << " msgSeq=" << hsMsgSeq << " fragmentOffset=" << hsFragmentOffset;
            stream << " (" << hsFragmentLength << " bytes)"; // << RawDataToHex(&data[25], recordLen - 25);
