...
bool ofpx::normalizeTableFeaturesMessage(const ByteRange &data, ByteList &results) {
    ...
    if (ptr[0] < OFP_VERSION_4 || (ptr[1] != OFPT_MULTIPART_REQUEST &&
        ptr[1] != OFPT_MULTIPART_REPLY)) {
        return false;
    }
...
static bool normalizeTableFeaturesEntry(const UInt8 *&ptr, size_t &remainingLength, ByteList &results) {
    ...
    for (auto next : props) {
        ...
        if (prev && prev->length == next->length && (prev->type & 0xFFFE) == (next->type & 0xFFFE) && std::memcmp(&prev->length, &next->length, next->length - 2) == 0) {
    ...
    *Big16_cast(results.mutableData() + start) = UInt16_narrow_cast(results.size() - start);
