...
*-R, --roundtrip*::
    After translating the input to a binary message, translate it back to YAML/JSON.
    This option is used to translate the YAML/JSON input into "canonical" form. Specify 
    a different output format by using the `-o` option to specify the output file name with a
    file extension: .yml, .yaml, or .json.
