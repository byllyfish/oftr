...
character. The maximum length of a message, including the newline, is 1 megabyte.
A JSON-RPC message will normally be a JSON mapping that begins with '{' and ends
with '}'.  Alternately, a JSON-RPC message may be enclosed in double quotes, and the contents
of the JSON string will be parsed as YAML.
...
*conn_id*:: Identifier for the outgoing connection used.

*data*:: Binary OpenFlow message sent.

==== Discussion

To send an OpenFlow message, use the `OFP.SEND` command. Omit the `id` parameter 
to prevent the reply from being sent back.
