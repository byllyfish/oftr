...
* Property names begin with a lower-case letter, e.g. `type`.

* Type names begin with a capital letter and use mixed case, e.g. `UInt8`, `Field`.

* Literal strings are all-caps, e.g. `PACKET_IN`.

* Sequences are presented as an array with one element, e.g. `[UInt8]`. If the array has more than one element, then you have a sequence where each element has a choice of types.
...
* Unrecognized property names are not permitted.

* On input to libofp, properties are allowed to be in any order.

* Messages output from libofp are always produced in the canonical encoding.

* !opt means the property may be omitted on input, but is always output in the canonical encoding.

* !optout means the property may be omitted, even in the canonical encoding. Client software should test for it.
