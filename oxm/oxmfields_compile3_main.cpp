...
int main()
{
	...
	std::sort(entries.begin(), entries.end(), 
		[](const OXMTypeInternalMapEntry &lhs, const OXMTypeInternalMapEntry &rhs) {
...
#include "ofp/oxmfields.h"
	...
	std::cout << "const ofp::OXMTypeInternalMapEntry ofp::OXMTypeInternalMapArray[] = {\n";
	for (auto entry : entries) {
		std::cout << "  { " << entry.value32 << ", ";
		std::cout << "static_cast<ofp::OXMInternalID>(" << static_cast<int>(entry.id) << ") },\n";
	}
	std::cout << "};\n";
}
