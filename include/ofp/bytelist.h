 ...
 */
inline const ofp::UInt8 *ofp::ByteList::begin() const { return &buf_[0]; }
 ...
 *  Return pointer to beginning of byte buffer.
 */
inline const ofp::UInt8 *ofp::ByteList::data() const { return &buf_[0]; }

/**
 *  Return pointer to beginning of byte buffer. Contents of buffer may be
 *  changed.
 */
inline ofp::UInt8 *ofp::ByteList::mutableData() { return &buf_[0]; }

/**
 *  Return size of byte buffer.
 */
inline size_t ofp::ByteList::size() const { return buf_.size(); }
 ...
 */
inline void ofp::ByteList::insertUninitialized(const UInt8 *pos,
                                               size_t length) {
  assertInRange(pos);
 ...
 */
inline void ofp::ByteList::resize(size_t length) { buf_.resize(length); }

/**
 *  Set size of byte buffer to zero.
 */
inline void ofp::ByteList::clear() { buf_.clear(); }
