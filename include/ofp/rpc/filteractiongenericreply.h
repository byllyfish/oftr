...
private:
    bool applyICMPv4(ByteRange enetFrame, PortNumber inPort, Message *message, OXMRange oxm);
    bool buildICMPv4(ByteRange enetFrame, OXMRange oxm, ByteList *reply);

    bool applyICMPv6(ByteRange enetFrame, PortNumber inPort, Message *message, OXMRange oxm);

    void sendPacketOut(const ByteList *data, PortNumber outPort, Message *message);
};
