...
struct SequenceTraits<ofp::TableIDRange> {
  ...
  static ofp::UInt8 &
  element(IO &io, ofp::TableIDRange &range, size_t index) {
...
struct SequenceTraits<ofp::TableIDList> {
  static size_t size(IO &io, ofp::TableIDList &list) {
    return 0;
  }

  static ofp::detail::TableIDInserter &
  element(IO &io, ofp::TableIDList &list, size_t index) {
