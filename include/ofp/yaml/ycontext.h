...
struct YamlContext {
    explicit YamlContext(Encoder *enc) : encoder{enc}, decoder{nullptr}, version{0}, pktMatch{false} { assert(validate()); }
    explicit YamlContext(Decoder *dec, UInt8 ver, bool pMatch) : encoder{nullptr}, decoder{dec}, version{ver}, pktMatch{pMatch} { assert(validate()); }
