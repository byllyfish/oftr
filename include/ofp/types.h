...
template <class T> inline T Unsigned_difference(T lhs, T rhs)
{
    return lhs >= rhs ? lhs - rhs : 0;
}

// Convenience function to check for literal type.
template <class T> constexpr bool IsLiteralType()
{
 ...
 */
template <class T> constexpr bool IsStandardLayout()
{
 ...
 */
template <class T, size_t N> constexpr size_t ArrayLength(T(&)[N])
{
    return N;
}
...
/**
 * Convert raw buffer to a hexadecimal string. The resulting string is formatted
 * according to `delimiter` and `word`. The delimiter specifies a character to
 * insert between each run of hexadecimal chars. `word` specifies the number of
...
/**
 * Convert a hexadecimal string to raw memory. Ignore non-hex digits and the odd
 * final hex digit.
 *
 * @param  hex string containing hexadecimal characters
 ...
 * @return true if memory block is filled with given byte value.
 */
bool MemFilled(const void *data, size_t length, char fill);
