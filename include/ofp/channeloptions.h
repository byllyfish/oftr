...
constexpr ChannelOptions operator&(ChannelOptions lhs, ChannelOptions rhs) {
    return static_cast<ChannelOptions>(static_cast<unsigned>(lhs) & static_cast<unsigned>(rhs));
}
...
inline bool AreChannelOptionsValid(ChannelOptions options) {
    ...
    if ((options & ChannelOptions::AUXILIARY) != 0 && (options & ChannelOptions::FEATURES_REQ) == 0)
        return false;
    // LISTEN_UDP requires AUXILIARY
    if ((options & ChannelOptions::LISTEN_UDP) != 0 && (options & ChannelOptions::AUXILIARY) == 0)
        return false;
    // CONNECT_UDP requires !FEATURES_REQ
    if ((options & ChannelOptions::CONNECT_UDP) != 0 && (options & ChannelOptions::FEATURES_REQ) != 0) 
        return false;
    return true;
}
