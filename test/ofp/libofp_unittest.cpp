...
TEST(libofp, oftr_encode) {
  ...
  char small[5];
  EXPECT_EQ(oftr_call(OFTR_ENCODE + vers, msg, msglen, small, sizeof(small)),
            -8);
}
...
TEST(libofp, oftr_decode) {
  ...
  int len =
      oftr_call(OFTR_DECODE, msg.data(), msg.size(), buffer, sizeof(buffer));
  EXPECT_EQ(len, 123);
  EXPECT_ASCII(
      "---\ntype:            HELLO\nxid:             0x00000000\nversion:      "
      "   0x01\nmsg:             \n  versions:        [  ]\n...\n",
  ...
  char small[5];
  EXPECT_EQ(
      oftr_call(OFTR_DECODE, msg.data(), msg.size(), small, sizeof(small)),
      -123);
}
