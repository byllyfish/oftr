...
TEST(dtls_utils, DTLS_PrintRecord) {
    ...
    EXPECT_EQ("DTLS1.0 handshake(22) len=14 epoch=0 seq=0 [truncated]", DTLS_PrintRecord(buf1.data(), buf1.size()));

    // DTLS1.2
    std::string buf2 = HexToRawData("16FEFD00000000000000000001FF");
    EXPECT_EQ("DTLS1.2 handshake(22) len=14 epoch=0 seq=0 [truncated]", DTLS_PrintRecord(buf2.data(), buf2.size()));
