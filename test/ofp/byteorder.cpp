...
TEST(byteorder, detail::IsHostBigEndian)
{
	...
	if (detail::IsHostBigEndian) {
		...
		EXPECT_NEQ(0, std::memcmp(le, &n, 4));
	} else {
		EXPECT_TRUE(detail::IsHostLittleEndian);
		EXPECT_EQ(0, std::memcmp(le, &n, 4));
		EXPECT_NEQ(0, std::memcmp(be, &n, 4));
	}
}

TEST(byteorder, detail::SwapTwoBytes)
{
	EXPECT_EQ(0x0201, detail::SwapTwoBytes(0x0102));
	EXPECT_EQ(0xf2f4, detail::SwapTwoBytes(0xf4f2));
}

TEST(byteorder, detail::SwapFourBytes)
{
	EXPECT_EQ(0x04030201, detail::SwapFourBytes(0x01020304));
	EXPECT_EQ(0xf2f4f8fa, defail::SwapFourBytes(0xfaf8f4f2));
}

TEST(byteorder, detail::SwapEightBytes)
{
...
{
	ubig16_t a = 0x0102030405060708ULL;
