...
static Deferred<int> startUnsaved() { return Deferred<int>::makeResult(); }
...
TEST(deferred, lifetimeTest1) {
  {
    ...
    result.done([](int value) { EXPECT_EQ(1, value); });
...
TEST(deferred, lifetimeTest2) {
  {
    ...
    result.done([](int value) { EXPECT_EQ(2, value); });
  }
...
TEST(deferred, testImmediateCall) {
  {
    ...
    result.done([](int value) { EXPECT_EQ(5, value); });
...
static Deferred<int> startImmediatelyReturn(int value) { return value; }

TEST(deferred, testImmediateReturn) {
  auto result = startImmediatelyReturn(7);

  result.done([](int value) { EXPECT_EQ(7, value); });
