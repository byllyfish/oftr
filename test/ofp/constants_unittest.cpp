...
TEST(constants, OFPActionTypeFlagsConvertToV1) {
  ...
  for (size_t i = 0; i < ArrayLength(actionTypes); ++i) {
    EXPECT_EQ(actionTypes[i].second, OFPActionTypeFlagsConvertToV1(actionTypes[i].first)) << "Failed at iteration " << i;
  }
}
...
TEST(constants, OFPActionTypeFlagsConvertFromV1) {
  ...
  for (size_t i = 0; i < ArrayLength(actionTypes); ++i) {
    if (actionTypes[i].second) {
        EXPECT_EQ(actionTypes[i].first, OFPActionTypeFlagsConvertFromV1(actionTypes[i].second)) << "Failed at iteration " << i;
    }
  }
}
